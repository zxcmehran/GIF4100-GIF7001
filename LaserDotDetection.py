...
def getPointCoordinates(img, color):
    ...
    if color == 'red':
        mask = getRedMask(imgHSV)
    else:
        print('Color not implemented.')
        sys.exit(1)

    # Filter reflections from door handle
    cleanedMask = cv2.fastNlMeansDenoising(mask, 20,29,51)

    if isDebug:
        cv2.imshow('not_cleaned_mask', mask)
        cv2.imshow('mask',cleanedMask)


    cnts = cv2.findContours(cleanedMask, cv2.RETR_EXTERNAL,
